...
var SkyPlusHD_Settings = {
   ...
   Services: {
      SkyServe    : "urn:schemas-nds-com:device:SkyServe",
      SkyBook     : "urn:schemas-nds-com:service:SkyBook",
      SkyBrowse   : "urn:schemas-nds-com:service:SkyBrowse",
      SkyControl  : "urn:schemas-nds-com:device:SkyControl",
      SkyPlay     : "urn:schemas-nds-com:service:SkyPlay",
      SkyCM       : "urn:schemas-nds-com:service:SkyCM",
      SkyRC       : "urn:schemas-nds-com:service:SkyRC"
   }
};
...
var SkyPlusHD = module.exports = function() {
   ...
   this.find = function SkyPlusHD_find(ipAddress) {
      var deferred = q.defer();
      //
      q.all([
         discoverService(SkyPlusHD_Settings.Services.SkyRC, ipAddress),
         discoverService(SkyPlusHD_Settings.Services.SkyBrowse, ipAddress)
      ]).then(function(vals) {
...
var SkyPlusHDBox = function (options) {
   ...
   var subscribe = function SkyPlusHDBox_subscribe() {
      ...
      SkyBoxRequest(requestOptions,function(err,msg,body) {
         if (err) {deferred.reject(err); console.log('error 1 %s',JSON.stringify(err));setTimeout(subscribe,SkyPlusHD_Settings.renewSubscriptionInterval);}
         else if (msg.statusCode != 200) {deferred.reject(new Error('Failed to subscribe, http status '+msg.statusCode));console.log('error 2%s',JSON.stringify(err));}
         else {
   ...
   //
   this.ch_up = function() {
      self.setChannel(self.currentChannel.number+1)
   };
   //
   this.ch_down = function() {
      self.setChannel(self.currentChannel.number-1)
   };
   //
   this.play = function() {
      ...
      self.soapRequest(SkyPlusHD_Settings.Services.SkyPlay,'Play',soapPayload)
         .fail(function(err) {
            deferred.reject(err);
         })
         .then(function(response) {
